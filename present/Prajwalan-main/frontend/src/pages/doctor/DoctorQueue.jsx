...
export default function DoctorQueue() {
    ...
    const callPatient = async (queueId) => {
        try {
            await api.post(`/doctor/queue/${queueId}/call`);
            toast.success('Patient called to cabin 📢');
            loadQueue();
        } catch (e) {
        ...
        <div>
                            ...
                            {filtered.length === 0 ? (
                                ...
                                filtered.map((p, idx) => (
                                    <tr key={p.queue_id} style={{
                                        ...
                                        borderLeft: p.is_emergency ? '4px solid var(--danger)' : 'none'
                                    }}>
                                        <td>
                                            <div style={{
                                                width: 32, height: 32, borderRadius: 8,
                                                background: p.is_emergency ? 'var(--danger)' : 'var(--navy-100)',
                                                color: p.is_emergency ? 'white' : 'var(--navy-800)',
                                        ...
                                        <td>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                                {p.is_emergency && <span className="badge badge-red">EMERGENCY</span>}
                                                {p.priority === 'High' && !p.is_emergency && <span className="badge badge-danger">Critical</span>}
                                                {p.priority === 'Medium' && <span className="badge badge-yellow">Priority</span>}
                                                {p.age > 60 && <span className="badge badge-indigo">Senior</span>}
                                                {p.age < 12 && <span className="badge badge-orange">Child</span>}
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                <div className="progress-bar" style={{ width: 40, height: 4 }}>
                                                    <div className="progress-fill" style={{ width: `${p.priority_score}%`, background: 'var(--navy-500)' }} />
                                                </div>
                                                <span style={{ fontSize: 12, fontWeight: 700 }}>{p.priority_score}</span>
                                            </div>
                                        </td>
                                        <td>
                                            <div style={{ display: 'flex', gap: 8 }}>
                                                {p.status === 'waiting' && (
                                                    <button onClick={() => callPatient(p.queue_id)} className="btn btn-primary btn-sm flex-1">
                                                        <Play size={12} /> Call
                                                    </button>
                                                )}
                                                {p.status === 'called' && (
            ...
            <div style={{ marginTop: 24, display: 'flex', gap: 24 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--slate-500)' }}>
                    <div style={{ width: 12, height: 4, borderRadius: 2, background: 'var(--danger)' }} /> Emergency Case
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--slate-500)' }}>
                    <div style={{ width: 12, height: 4, borderRadius: 2, background: 'var(--warning)' }} /> Priority Appointment
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--slate-500)' }}>
                    <div style={{ width: 12, height: 4, borderRadius: 2, background: 'var(--navy-200)' }} /> Standard Queue
                </div>
            </div>
        </div>
    );
