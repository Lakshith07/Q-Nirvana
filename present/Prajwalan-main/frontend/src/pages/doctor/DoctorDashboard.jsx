...
export default function DoctorDashboard() {
    ...
    const [notifications, setNotifications] = useState([
        { id: 1, type: 'danger', title: 'New Emergency request assigned', sub: 'Patient: Rahul Sharma · 2 mins ago' },
        { id: 2, type: 'warning', title: 'Bed availability updated in ICU', sub: '1 bed vacant · 15 mins ago' },
        { id: 3, type: 'info', title: 'Lab results ready for Patient #102', sub: 'Hematology report · 1 hour ago' }
    ]);
                            ...
                            queue.slice(0, 5).map((p, idx) => (
                                <div key={p.queue_id} style={{
                                    ...
                                    <div style={{
                                        ...
                                        background: p.is_emergency ? 'var(--danger)' : 'var(--navy-700)',
                                    ...
                                    <div style={{ flex: 1 }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                            ...
                                            {p.is_emergency && <span className="badge badge-red" style={{ fontSize: 9 }}>Emergency</span>}
                                        </div>
                                    ...
                                    <div style={{ textAlign: 'right', display: 'flex', alignItems: 'center', gap: 12 }}>
                                        <div style={{ textAlign: 'right' }}>
                                            <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--slate-900)' }}>{p.priority}</div>
                                            <div style={{ fontSize: 11, color: 'var(--slate-400)' }}>Score: {p.priority_score}</div>
                                        </div>
                                        <button
                                            onClick={async () => {
                                                try {
                                                    await api.post(`/doctor/queue/${p.queue_id}/call`);
                                                    toast.success(`Calling ${p.patient_name} 📢`);
                                                    loadData(true);
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                    ...
                    <div className="card" style={{ background: '#eff6ff', border: '1px solid #bfdbfe' }}>
                        <div className="card-body" style={{ display: 'flex', gap: 16 }}>
                            <div style={{ width: 50, height: 50, borderRadius: 12, background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 10px rgba(37,99,235,0.1)', flexShrink: 0 }}>
                                <AlertCircle size={24} color="var(--navy-600)" />
                            </div>
                            <div>
                                <h4 style={{ fontSize: 15, fontWeight: 700, color: 'var(--navy-900)', marginBottom: 4 }}>Did you know?</h4>
                                <p style={{ fontSize: 13, color: 'var(--slate-600)', lineHeight: 1.5, margin: 0 }}>
                                    The queue algorithm automatically prioritizes Maternity visits (🤱) and Senior Citizens (👴) to ensure timely care.
                                </p>
                            </div>
                        </div>
                    </div>
