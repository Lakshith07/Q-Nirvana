...
export default function AdminQueue() {
    ...
    const getPriorityBadge = (score) => {
        if (score >= 100) return <span className="badge badge-red pulse">Emergency (100)</span>;
        if (score >= 80) return <span className="badge badge-orange">Maternity (80)</span>;
        if (score >= 70) return <span className="badge badge-yellow">Senior (70+)</span>;
        if (score >= 60) return <span className="badge badge-blue">Child (60)</span>;
        return <span className="badge badge-gray">General (50)</span>;
    };
        ...
        <div>
            <div className="flex justify-between items-end mb-8">
                <div>
                    ...
                    <p className="section-sub">Real-time prioritized monitoring of all outpatient departments</p>
                </div>
            ...
            <div className="card">
                                ...
                                {loading && queue.length === 0 ? (
                                    <tr>
                                        <td colSpan="6" className="text-center py-20">
                                            ...
                                            <p className="text-muted">Analyzing prioritize patient data...</p>
                                        </td>
                                    </tr>
                                    ...
                                    filteredQueue.map(q => (
                                        <tr key={q.id}>
                                            <td>{getPriorityBadge(q.priority_score)}</td>
                                            <td>
                ...
                <div className="card-footer bg-slate-50 p-4 border-t border-slate-100 italic text-[11px] text-slate-500">
                    * Queue order is calculated using the Intelligent Triage Algorithm (ITA) based on clinical severity and arrival time.
                </div>
            </div>
        </div>
    );
